...
def install_requirements(omit_libraries=None, disable_installation=False, dev_mode=False, dev_runs=50):
    """
    ...
    def create_dev_file():
        dev_data = {
            'runs': 0,
            'last_install': datetime.now().isoformat(),
            'max_runs': dev_runs
        }
        ...
        try:
            ...
            if os.path.exists(dev_file):
                try:
                    ...
                    dev_data = create_dev_file()
            else:
                # File doesn't exist - create it
                dev_data = create_dev_file()

            current_runs = dev_data.get('runs', 0) + 1
            
            if current_runs < dev_data.get('max_runs', dev_runs):
                # Update run count and skip installation
                dev_data['runs'] = current_runs
                with open(dev_file, 'w') as f:
                    json.dump(dev_data, f, indent=4)
                return

            # If we get here, we've hit the run limit
            if not disable_installation:
                # Your existing installation code here...
                pass
                
            # Reset the dev file after installation
            create_dev_file()

        except Exception as e:
            print(f"Dev mode error: {str(e)}. Proceeding with installation.")
            if not disable_installation:
                # Your existing installation code here...
                pass
            # Attempt to reset the dev file
            create_dev_file()
    else:
        # Non-dev mode: proceed with normal installation if not disabled
        if not disable_installation:
            # Your existing installation code here...
            pass
    ...
    # If in dev mode, create the marker file after successful installation
    if dev_mode:
        with open('.dev_mode', 'w') as f:
            f.write(f"Development mode enabled on {datetime.now()}")
